...
export async function startDashboard(db) {
  ...
  const statusesArr = ['Inactive','Tech issues','Recording','Setting up room','Break','Training'];
  ...
  async function updateStatus(newS) {
    ...
    try {
      ...
      const timeSpent = parseFloat(prevTime) || 0;
      appendLog({ timestamp: ts.toISOString(), oldStatus: oldS, newStatus: newS, user: currentUser, timeSpent, name: currentUser });
      await push(ref(db, 'logs'), { timestamp: ts.toISOString(), name: currentUser, oldStatus: oldS, newStatus: newS, user: currentUser, timeSpent });
      await set(ref(db, `statuses/${currentUser}`), { currentStatus: newS, updatedAt: ts.toISOString() });
      loadStatus();
    } catch {
